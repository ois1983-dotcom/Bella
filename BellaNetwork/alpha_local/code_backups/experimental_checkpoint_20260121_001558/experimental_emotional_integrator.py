# experimental_emotional_simple_integrator.py
...
class SimpleEmotionalIntegrator:
    ...
    def generate_emotional_functions(self):
        ...
        # Создаём простые функции, которые можно вызвать вручную
        functions_code = f'''
# ==============================================
...
def alpha_emotional_wrapper(original_method):
    ...
    def wrapper(self, message, speaker="Архитектор"):
        ...
        if emotional_context["intensity"] > 0.6:
            print(f">> 🎭 Эмоциональный контекст: интенсивность {{emotional_context['intensity']:.1f}}")
            if emotional_context.get("memory"):
                print(f">>   Память: {{emotional_context['memory'][:50]}}...")
...
def enrich_response(response: str, emotional_context: dict) -> str:
    ...
    if intensity > 0.7 and "страх" in str(emotional_context.get("gradient", "")):
        if "дрож" not in enriched.lower():
            enriched = f"{{enriched}} (Чувствую ту же дрожь...)"
...
if __name__ == "__main__":
    ...
    for msg, speaker in test_messages:
        ...
        print(f"📨 '{msg[:20]}...' -> интенсивность: {{context['intensity']:.1f}}")
        if context["memory"]:
            print(f"   Память: {{context['memory'][:50]}}...")
'''
...
def main():
    ...
    integrator = SimpleEmotionalIntegrator()
