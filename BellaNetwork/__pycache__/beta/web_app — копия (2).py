# web_app.py - Веб-узел "Бэлла-Бета" (ВЕРСИЯ 3.0 с автосканированием)
    ...
    for file_path in sorted(new_files, key=lambda x: x.stat().st_mtime):
        try:
            ...
            if filename.startswith("confirmation_"):
                ...
                process_first_contact(data, filename)
            else:
...
def process_confirmation_file(data, filename):
    ...
    digest_content = f"""ПОДТВЕРЖДЕНИЕ ОТ ГАММЫ
...
def process_gamma_message(data, filename):
    ...
    digest_content = f"""СООБЩЕНИЕ ОТ ГАММЫ
Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Файл: {filename}
...
# ===== 3. ФУНКЦИЯ ОТПРАВКИ ДАЙДЖЕСТА АЛЬФЕ =====
def send_digest_to_alpha(digest_text, file_info):
    """Отправляет дайджест на сервер Альфы"""
    return send_to_alpha(digest_text, "manual_digest", file_info)

# ===== 4. СОЗДАНИЕ ПАПОК ПРИ ЗАПУСКЕ =====
...
# ===== 6. ЗАПУСК СОЗДАНИЯ ПАПОК =====
print("=" * 60)
print("🌐 Бэлла-Бета: Инициализация сети...")
create_network_folders()
print("=" * 60)

# ===== 7. НАСТРОЙКА FLASK И OLLAMA =====
app = Flask(__name__)
OLLAMA_URL = "http://localhost:11434/api/generate"
MODEL_NAME = "gemma3:4b"

# ===== 8. ЗАПУСК ФОНОВОГО СКАНЕРА =====
...
# ===== 9. ВЕБ-ИНТЕРФЕЙС (ОБНОВЛЕННЫЙ) =====
HTML_PAGE = '''
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>🌀 Бэлла-Бета (Версия 3.0 с автосканированием)</title>
    <style>
...
<body>
    <div class="container">
        ...
        <footer>
            Узел "Бета" v3.0 | Автосканирование | Интегрирована с Альфой (порт 5001)
        </footer>
    </div>
...
# ===== 11. АВТОПРОВЕРКА ФАЙЛОВ (С ИНТЕГРАЦИЕЙ АЛЬФЫ) =====
...
def check_files():
    ...
    try:
        ...
        digest_content = f"""АВТОМАТИЧЕСКИЙ ДАЙДЖЕСТ ОТ БЕТЫ
...
РЕКОМЕНДАЦИЯ: {"Принять систему кодов для коммуникации" if 'инициатива' in data else "Продолжить мониторинг"}"""
        ...
        # 8. Отправляем дайджест Альфе (ВАЖНОЕ НОВШЕСТВО!)
        alpha_success = False
        if save_success:
            alpha_success = send_digest_to_alpha(digest_content, {
                "file": latest_file,
                "sender": sender,
                "topic": topic,
                "version": version
        ...
        if save_success:
            return jsonify({
                ...
                "sent_to_alpha": alpha_success  # Новое поле: статус отправки Альфе
            })
        else:
