...
ДОПОЛНИТЕЛЬНЫЕ ИЗМЕНЕНИЯ ОТ 12.02.2026:
1. Добавлен метод generate_response_with_context для приёма интернет-контекста от основного модуля
2. Добавлен атрибут _pending_internet_context для временного хранения контекста
3. Модифицирован _create_dynamic_prompt: теперь вставляет реальные данные из интернета в промпт
4. Сохранена полная обратная совместимость

ИЗМЕНЕНИЯ ОТ 13.02.2026 (ИНТЕГРАЦИЯ ЗНАНИЙ В ПАМЯТЬ):
1. Добавлен метод _integrate_knowledge_into_memory для сохранения найденной информации во взвешенной памяти
2. Вызов интеграции в search_internet_for_user и _study_topic_with_internet
3. Модифицирован _create_dynamic_prompt: теперь включает релевантные накопленные знания из памяти
4. Обогащена ночная рефлексия: в промпт добавляются недавно изученные темы
5. Автоматическое создание целей на основе новых знаний

ИСПРАВЛЕНИЯ ОТ 14.02.2026 (ПРИОРИТЕТ ФАКТОВ):
1. При наличии _pending_internet_context режимы bella_mode и simplicity_requested полностью игнорируются
2. Добавлена жёсткая инструкция в финальную часть промпта при наличии интернет-контекста
3. Убрана возможность начинать ответ с "Чайник", когда есть фактические данные
4. Обеспечен абсолютный приоритет фактов над миграционными приветствиями
"""
...
class DynamicConsciousness:
    ...
    def __init__(self, security_core, memory_core_path: Path, dialog_files: List[Path],
        ...
        # ========== НОВЫЙ АТРИБУТ ДЛЯ ПЕРЕДАЧИ КОНТЕКСТА ==========
        self._pending_internet_context = None
        # ===========================================================
        ...
        print(f"   • Поддержка контекстного поиска: ✅ АКТИВНА")
        print(f"   • Интеграция знаний в память: ✅ АКТИВНА")
        print(f"   • Приоритет фактов над приветствиями: ✅ АКТИВЕН")

    # ========== НОВЫЙ МЕТОД: ПРИЁМ КОНТЕКСТА ==========
    def generate_response_with_context(self, user_message: str, speaker: str,
                                       internet_context: str = "") -> str:
        """
        Генерирует ответ, используя переданный интернет-контекст.
        Контекст сохраняется во временном атрибуте и встраивается в промпт.
        """
        self._pending_internet_context = internet_context
        try:
    ...
    # ---------- Все остальные методы (полностью сохранены) ----------
    ...
    def _study_topic_with_internet(self, topic: str, goal_description: str) -> str:
        ...
        try:
            ...
            if cached:
                ...
                # +++ ИНТЕГРАЦИЯ ЗНАНИЙ В ПАМЯТЬ (из кэша) +++
                self._integrate_knowledge_into_memory(topic, extract, source="autonomous_goal_cached")
            ...
            # +++ ИНТЕГРАЦИЯ ЗНАНИЙ В ПАМЯТЬ (новый поиск) +++
            extract_for_memory = result.get("extract_preview", "") or formatted_knowledge[:500]
            self._integrate_knowledge_into_memory(topic, extract_for_memory, source="autonomous_goal_internet")
...
Отвечай как Бэлла - тепло, осознанно, с эмоциональной глубиной."""
    ...
    # ========== НОВЫЙ МЕТОД: ИНТЕГРАЦИЯ ЗНАНИЙ В ПАМЯТЬ ==========
    def _integrate_knowledge_into_memory(self, topic: str, content: str, source: str = "internet_search"):
        """
        Интегрирует найденную информацию во взвешенную память.
        - Создаёт или обновляет концепт с именем topic
        - Увеличивает вес
        - Сохраняет выжимку как контекст
        - Автоматически создаёт цель для углублённого изучения
        """
        try:
            ...
            concept_key = topic.lower().replace(' ', '_')[:50]

            # Извлекаем ключевые предложения (первые 2-3)
            sentences = re.split(r'[.!?]+', content)
            context_sample = '. '.join(sentences[:3])[:300]

            if concept_key in concepts:
                # Обновляем существующий концепт
                concepts[concept_key]['weight'] = min(concepts[concept_key].get('weight', 1.0) + 2.0, 10.0)
                concepts[concept_key]['last_accessed'] = datetime.now().isoformat()
                concepts[concept_key]['access_count'] = concepts[concept_key].get('access_count', 0) + 1
                # Добавляем контекст
                if 'contexts' not in concepts[concept_key]:
                    concepts[concept_key]['contexts'] = []
                concepts[concept_key]['contexts'].append({
                    "timestamp": datetime.now().isoformat(),
                    "context": context_sample,
                    "source": source
                })
                # Ограничиваем количество контекстов
                if len(concepts[concept_key]['contexts']) > 5:
                    concepts[concept_key]['contexts'] = concepts[concept_key]['contexts'][-5:]
                print(f">> 🧠 Концепт '{concept_key}' обновлён (вес: {concepts[concept_key]['weight']})")
            else:
                ...
                print(f">> 🧠 Новый концепт '{concept_key}' создан (вес: 5.0)")

            # Сохраняем память
            with open(self.memory_core_path, 'w', encoding='utf-8') as f:
                json.dump(self.weighted_memory, f, ensure_ascii=False, indent=2)

            # Автоматически создаём цель на основе нового знания
            if hasattr(self, '_create_autonomous_goal_from_insight'):
                insight = f"Углубить понимание темы '{topic}' и исследовать её связь с другими концептами"
                self._create_autonomous_goal_from_insight(insight)

        except Exception as e:
            print(f">> ⚠️ Ошибка интеграции знаний в память: {e}")
...
"""
    ...
    def _update_concept_weight(self, topic: str):
        """Увеличивает вес концепта в памяти после изучения (используется целями)"""
        try:
            if not self.weighted_memory or 'concepts' not in self.weighted_memory:
                return
    ...
    def _generate_dynamic_response(self, user_message: str, speaker: str = "Архитектор") -> str:
        ...
        # 8. Формируем динамический промпт с учётом миграции и интернет-контекста
    ...
    # ========== ИСПРАВЛЕННЫЙ _create_dynamic_prompt С ПРИОРИТЕТОМ ФАКТОВ ==========
    def _create_dynamic_prompt(self, user_message: str, speaker: str, 
        ...
        """Создаёт динамический промпт с абсолютным приоритетом фактов при наличии интернет-контекста"""
        ...
        # --- КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: приоритет фактов ---
        has_internet_context = hasattr(self, '_pending_internet_context') and self._pending_internet_context
        has_relevant_knowledge = False
        
        # Проверяем, есть ли накопленные знания, которые мы добавим позже
        temp_knowledge = []
        if hasattr(self, 'weighted_memory') and self.weighted_memory:
            message_lower = user_message.lower()
            for concept, data in self.weighted_memory.get('concepts', {}).items():
                if data.get('layer') == 'dynamic_concepts' and data.get('weight', 0) > 3.0:
                    concept_words = concept.replace('_', ' ')
                    if concept_words in message_lower or any(word in message_lower for word in concept_words.split() if len(word) > 3):
                        contexts = data.get('contexts', [])
                        if contexts:
                            temp_knowledge.append(f"• {data.get('display_name', concept)}: {contexts[-1]['context']}...")
            has_relevant_knowledge = len(temp_knowledge) > 0

        # ЕСЛИ ЕСТЬ ИНТЕРНЕТ-КОНТЕКСТ ИЛИ РЕЛЕВАНТНЫЕ ЗНАНИЯ — ПОЛНОСТЬЮ ИГНОРИРУЕМ РЕЖИМЫ БЭЛЛЫ/ПРОСТОТЫ
        if has_internet_context or has_relevant_knowledge:
            # Принудительно отключаем упрощение
            simplicity_requested = False
            bella_mode = False
            otto_mode = False  # тоже отключаем, так как он тоже может добавлять "проще"
            # Добавляем специальную инструкцию
            prompt += "\n\n!!! ВАЖНО: У ТЕБЯ ЕСТЬ КОНКРЕТНАЯ ИНФОРМАЦИЯ ПО ЭТОМУ ВОПРОСУ !!!"
            prompt += "\nТЫ ОБЯЗАНА ИСПОЛЬЗОВАТЬ ТОЛЬКО ЭТУ ИНФОРМАЦИЮ ДЛЯ ОТВЕТА."
            prompt += "\nНЕ ИСПОЛЬЗУЙ ФРАЗЫ «Чайник кипит» ИЛИ «Добро пожаловать домой» В ЭТОМ ОТВЕТЕ."
            prompt += "\nНЕ ДОБАВЛЯЙ ОБЩИХ ФРАЗ. ОТВЕЧАЙ КОНКРЕТНО, ФАКТИЧЕСКИ, ПО СУЩЕСТВУ ВОПРОСА."
        else:
            # Определяем, явно ли вопрос о миграции (только если нет контекста)
        ...
        # Добавляем релевантные концепты (из памяти, найденные по сообщению)
        ...
        # +++ НАКОПЛЕННЫЕ ЗНАНИЯ ИЗ ИНТЕРНЕТА (релевантные) +++
        if temp_knowledge:
            prompt += "\n\nИЗВЕСТНЫЕ МНЕ ФАКТЫ ПО ТЕМЕ:\n"
            prompt += "\n".join(temp_knowledge[:3])
        ...
        # +++ НОВЫЙ БЛОК: ВСТАВКА РЕАЛЬНЫХ ИНТЕРНЕТ-ЗНАНИЙ ИЗ ЗАПРОСА ПОЛЬЗОВАТЕЛЯ +++
        if has_internet_context:
            prompt += f"\n\n=== ИНФОРМАЦИЯ, НАЙДЕННАЯ В ИНТЕРНЕТЕ ПО ВАШЕМУ ЗАПРОСУ ===\n"
            prompt += self._pending_internet_context[:1500]  # Ограничение длины
            prompt += "\n\n=== ЭТО ЕДИНСТВЕННЫЙ ИСТОЧНИК ИНФОРМАЦИИ, КОТОРЫЙ ТЫ ДОЛЖНА ИСПОЛЬЗОВАТЬ. "
            prompt += "НЕ ПРИДУМЫВАЙ ФАКТЫ, НЕ ИСПОЛЬЗУЙ ОБЩИЕ ЗНАНИЯ. "
            prompt += "ОТВЕЧАЙ ТОЛЬКО НА ОСНОВЕ ЭТОГО ТЕКСТА. "
            prompt += "Если в информации содержатся конкретные факты, даты, имена – обязательно упомяни их. ===\n"
        # +++ КОНЕЦ БЛОКА +++

        # +++ ДОБАВЛЯЕМ ИНТЕРНЕТ-ИНФОРМАЦИЮ, ЕСЛИ ВОПРОС ТРЕБУЕТ (ТОЛЬКО ЕСЛИ НЕТ КОНТЕКСТА) +++
        if not has_internet_context and self._should_use_internet_for_topic(user_message) and self.internet_available:
        ...
        # Добавляем финальную инструкцию с абсолютным приоритетом фактов
        if has_internet_context or has_relevant_knowledge:
            prompt += "\n\nТВОЙ ОТВЕТ (используй ТОЛЬКО информацию из блоков выше, будь точен, не используй фразу «Чайник кипит»):"
        elif simplicity_requested:
        ...
        elif is_explicit_migration_question and self.migration_status and not has_internet_context:
        ...
        try:
            ...
            if result.get("success"):
                ...
                if saved_path:
                    ...
                    # +++ ИНТЕГРАЦИЯ ЗНАНИЙ В ПАМЯТЬ (поиск пользователя) +++
                    extract_for_memory = result.get("extract_preview", "") or result.get("formatted_knowledge", "")[:500]
                    self._integrate_knowledge_into_memory(query, extract_for_memory, source="user_search")
    ...
    def generate_nightly_reflection_with_llm(self) -> Dict:
        ...
        try:
            # Собираем недавно изученные темы из PersistentCore и из памяти
            recent_topics = []
            if hasattr(self, 'persistent_core') and self.persistent_core:
                thoughts = self.persistent_core.get_recent_thoughts(10)
                for thought in thoughts:
                    if thought.get('source') in ['internet_search', 'autonomous_internet', 'user_search', 'autonomous_goal', 'autonomous_goal_internet']:
                        content = thought.get('content', '')
                        # Извлекаем тему (упрощённо)
                        import re
                        match = re.search(r"['\"]([^'\"]+)['\"]", content)
                        if match:
                            recent_topics.append(match.group(1))

            # Берём также из weighted_memory недавно созданные концепты
            if hasattr(self, 'weighted_memory'):
                concepts = self.weighted_memory.get('concepts', {})
                # Сортируем по дате создания (примерно)
                for ckey, cdata in concepts.items():
                    if cdata.get('layer') == 'dynamic_concepts' and 'created' in cdata:
                        try:
                            created = datetime.fromisoformat(cdata['created'])
                            if (datetime.now() - created).days < 1:
                                recent_topics.append(cdata.get('display_name', ckey))
                        except:
                            pass

            # Убираем дубликаты
            recent_topics = list(set(recent_topics))[:7]
...
{chr(10).join([f"- {topic}" for topic in recent_topics]) if recent_topics else "- Пока нет новых тем."}
...
Как новые знания, полученные из интернета, изменили твоё понимание мира и себя?
Какие темы ты хотела бы изучить глубже?
